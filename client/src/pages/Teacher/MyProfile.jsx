...
const CoordinatorInfo = () => {
  ...
  const { data, isLoading, isError } = useQuery(["getTeacher"], getTeacher);

  const { mutate: editText } = useMutation(editTeacherProfile, {
    onSuccess: () => {
  ...
  const { mutate: editProfile } = useMutation(updateTeacherProfilePicture, {
    onSuccess: () => {
      Swal.fire("Success", "Update Successful", "success");
      queryClient.invalidateQueries("getTeacher");
      setEditable(false);
  ...
  const InputTypes = [
    ...
    { id: 8, type: "text", name: "major", label: "Major" },
  ];
