...
import { getCompanyList, getCoordinatorList, getStudentList, getTeacherList, getTrainerList } from "../../api/Api";
...
const Dashboard = () => {
  ...
  const  {
    data: trainerList, isLoading: trainerLoading
  } = useQuery({
  ...
  const totalNewStudent = StudentList ? StudentList .filter((item) => item.deletedStatus ===0).length : [];
  const totalOldStudent = StudentList ? StudentList .filter((item) => item.deletedStatus ===1).length : [];
...
const graphData = [
  ...
  if( companyLoading || coordinatorLoading|| teacherLoading || trainerLoading || studentLoading) {
        ...
        <div className="w-full flex flex-col">
         ...
         <section className="grid grid-cols-4 grid-rows-5 gap-3">
            <div className="col-span-2 h-[160px] row-span-2 grid grid-cols-3 gap-[1.2px] bg-violet-100 rounded-lg shadow-xl overflow-hidden shadow-[#f4f2f2] border border-[#ecf0f1]">
              {
                countBox.map((item, i) => (
                  <div key={i} className="bg-white flex items-center justify-between px-5">
                    ...
                    <div className="flex flex-col">
                      ...
                      <small className="text-gray-500 text-[0.7rem]">{item.label}</small>
                    </div>
                  </div>
                ))
              }
            ...
            <LineChart data={graphData} sizeHeight={300} years={Object.keys(getBatchStudent)}/>
            </div>




            <div className="row-span-3 bg-white rounded-lg p-3 shadow-xl overflow-hidden shadow-[#f4f2f2] border border-[#ecf0f1] text-gray-500 flex flex-col items-center py-7">Under Development <span className="text-red-500 text-3xl">--</span></div>
            <div className="row-span-3 bg-white rounded-lg p-3 shadow-xl overflow-hidden shadow-[#f4f2f2] border border-[#ecf0f1] text-gray-500 flex flex-col items-center py-7">Under Development <span className="text-red-500 text-3xl">--</span></div>
         </section>
