...
function TimeOut() {
  ...
  return (
    <div>
      ...
      {
             taskFile == null? (
                <div className={` ${!canTimeOut && 'cursor-not-allowed opacity-50' } relative px-4 py-3 rounded bg-blue-500 mb-5 w-[180px]  flex items-center justify-center gap-2`} >
                    <input
                    type="file"
                    accept=".pdf,.doc,.docx,.txt"
                    onChange={handleFileChange}
                    className={`${!canTimeOut && 'cursor-not-allowed' } mb-2 absolute w-full h-full cursor-pointer opacity-0`}
                    disabled={!canTimeOut}
                    />
                    <ImAttachment className="text-white" />
                    <span className="text-white font-medium text-sm tracking-wide">Upload task</span>
                </div>
            ) : (
       ...
       {
            !canTimeOut && <div className="flex items-center gap-2"><BiError /> <span className="font-semibold">Error:</span> Time Out not allowed. Please record a Time In before proceeding.</div>
        }
       {
            taskFile == null && <span className="flex items-center gap-2"><BiError /> <span className="font-semibold">Error:</span> Please attach a task before proceeding to Time Out </span>
        }
       </div>
      </div>
    </div>
