...
const trainerData = {
  ...
  const handleAddrainer = async (e) => {
    e.preventDefault();
    
    try {
      const response = await axios.post('http://localhost:3001/addTrainer', trainerData);
      console.log({generate_password: response.data})
    } catch(error) {
        console.log(error);
    }
  }
